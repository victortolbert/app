  ...
  <section class="container">
    <div>
      ...
      <div class="links">
        ...
        <a
          :class="
            needWorkerSetup || !longRunningWorkers.length ? 'hidden' : 'visible'
          "
          class="button button--green"
          @click="freeWorker"
        >
        ...
        <a class="button button--grey" @click="removeWorker">
          Remove Web Worker
        </a>
        <a class="button button--grey" @click="createWorkers">
          Create more Workers
        </a>
      </div>
    </div>
...
export default {
  ...
  watch: {
    workers(workers) {
      if (workers.length === 0)
        this.notification =
          'Zero free Web Workers - click "Create more Workers"'
    },
  },
  methods: {
    test() {
    ...
    removeWorker() {
      ...
      if (!worker) return
      if (this.longRunningWorkers.includes(worker))
        this.longRunningWorkers.splice(
          this.longRunningWorkers.indexOf(worker),
          1,
        )
    ...
    createWorkers() {
      if (process.client) {
        for (
          let i = 0, len = navigator.hardwareConcurrency || 1;
          i < len;
          i++
        ) {
          this.workers.push(this.$worker.createWorker())
