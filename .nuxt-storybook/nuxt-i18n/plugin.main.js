...
export default async context => {
  ...
  const doDetectBrowserLanguage = route => {
    ...
    if (
      finalLocale &&
      (!useCookie || alwaysRedirect || !app.i18n.getLocaleCookie())
    ) {
  ...
  const extendVueI18nInstance = i18n => {
    ...
    i18n.getLocaleCookie = () =>
      getLocaleCookie(req, {useCookie, cookieKey, localeCodes})
  ...
  const vueI18nOptions =
    typeof vueI18n === 'function' ? vueI18n(context) : klona(vueI18n)
  ...
  if (!finalLocale) {
    if (
      ...
      const options = {
        localDomainKey: LOCALE_DOMAIN_KEY,
