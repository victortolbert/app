<script>
import {capitalize} from '~/helpers'

export default {
  props: {
    name: {
      type: String,
      default: 'question-mark-circle',
    },
    label: {
      type: String,
      default: null,
    },
    library: {
      type: String,
      default: 'heroicon',
    },
    isMedium: {
      type: Boolean,
      default: false,
    },
  },
  computed: {
    title() {
      return this.label || capitalize(this.name)
    },
    viewBox() {
      return this.isMedium ? '0 0 24 24' : '0 0 20 20'
    },
    description() {
      return capitalize(`${this.library}: ${this.name}`)
    },
    width() {
      return this.isMedium ? 24 : 20
    },
    height() {
      return this.isMedium ? 24 : 20
    },
  },
}
</script>

<template>
  <svg
    :width="height"
    :height="height"
    :x-description="description"
    :view-box="viewBox"
    fill="currentColor"
    aria-hidden="true"
  >
    <title>{{ title }}</title>

    <path
      v-if="name === 'double-back'"
      fill-rule="evenodd"
      d="M15.707 15.707a1 1 0 01-1.414 0l-5-5a1 1 0 010-1.414l5-5a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 010 1.414zm-6 0a1 1 0 01-1.414 0l-5-5a1 1 0 010-1.414l5-5a1 1 0 011.414 1.414L5.414 10l4.293 4.293a1 1 0 010 1.414z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'single-back'"
      fill-rule="evenodd"
      d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z"
      clip-rule="evenodd"
    />

    <g v-if="name === 'double-fwd'">
      <path
        fill-rule="evenodd"
        d="M10.293 15.707a1 1 0 010-1.414L14.586 10l-4.293-4.293a1 1 0 111.414-1.414l5 5a1 1 0 010 1.414l-5 5a1 1 0 01-1.414 0z"
        clip-rule="evenodd"
      />
      <path
        fill-rule="evenodd"
        d="M4.293 15.707a1 1 0 010-1.414L8.586 10 4.293 5.707a1 1 0 011.414-1.414l5 5a1 1 0 010 1.414l-5 5a1 1 0 01-1.414 0z"
        clip-rule="evenodd"
      />
    </g>

    <path
      v-if="name === 'single-fwd'"
      fill-rule="evenodd"
      d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'academic-cap'"
      d="M10.394 2.08a1 1 0 00-.788 0l-7 3a1 1 0 000 1.84L5.25 8.051a.999.999 0 01.356-.257l4-1.714a1 1 0 11.788 1.838L7.667 9.088l1.94.831a1 1 0 00.787 0l7-3a1 1 0 000-1.838l-7-3zM3.31 9.397L5 10.12v4.102a8.969 8.969 0 00-1.05-.174 1 1 0 01-.89-.89 11.115 11.115 0 01.25-3.762zM9.3 16.573A9.026 9.026 0 007 14.935v-3.957l1.818.78a3 3 0 002.364 0l5.508-2.361a11.026 11.026 0 01.25 3.762 1 1 0 01-.89.89 8.968 8.968 0 00-5.35 2.524 1 1 0 01-1.4 0zM6 18a1 1 0 001-1v-2.065a8.935 8.935 0 00-2-.712V17a1 1 0 001 1z"
    />

    <path
      v-if="name === 'adjustments'"
      d="M5 4a1 1 0 00-2 0v7.268a2 2 0 000 3.464V16a1 1 0 102 0v-1.268a2 2 0 000-3.464V4zM11 4a1 1 0 10-2 0v1.268a2 2 0 000 3.464V16a1 1 0 102 0V8.732a2 2 0 000-3.464V4zM16 3a1 1 0 011 1v7.268a2 2 0 010 3.464V16a1 1 0 11-2 0v-1.268a2 2 0 010-3.464V4a1 1 0 011-1z"
    />

    <path
      v-if="name === 'annotation'"
      fill-rule="evenodd"
      d="M18 13V5a2 2 0 00-2-2H4a2 2 0 00-2 2v8a2 2 0 002 2h3l3 3 3-3h3a2 2 0 002-2zM5 7a1 1 0 011-1h8a1 1 0 110 2H6a1 1 0 01-1-1zm1 3a1 1 0 100 2h3a1 1 0 100-2H6z"
      clip-rule="evenodd"
    />

    <g v-if="name === 'archive'">
      <path d="M4 3a2 2 0 100 4h12a2 2 0 100-4H4z" />
      <path
        fill-rule="evenodd"
        d="M3 8h14v7a2 2 0 01-2 2H5a2 2 0 01-2-2V8zm5 3a1 1 0 011-1h2a1 1 0 110 2H9a1 1 0 01-1-1z"
        clip-rule="evenodd"
      />
    </g>

    <path
      v-if="name === 'arrow-circle-down'"
      fill-rule="evenodd"
      d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11a1 1 0 10-2 0v3.586L7.707 9.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L11 10.586V7z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'arrow-top-right'"
      d="M20 4h1a1 1 0 00-1-1v1zm-1 12a1 1 0 102 0h-2zM8 3a1 1 0 000 2V3zM3.293 19.293a1 1 0 101.414 1.414l-1.414-1.414zM19 4v12h2V4h-2zm1-1H8v2h12V3zm-.707.293l-16 16 1.414 1.414 16-16-1.414-1.414z"
    />

    <path
      v-if="name === 'badge-check'"
      fill-rule="evenodd"
      d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 2.812c.051.643.304 1.254.723 1.745a3.066 3.066 0 010 3.976 3.066 3.066 0 00-.723 1.745 3.066 3.066 0 01-2.812 2.812 3.066 3.066 0 00-1.745.723 3.066 3.066 0 01-3.976 0 3.066 3.066 0 00-1.745-.723 3.066 3.066 0 01-2.812-2.812 3.066 3.066 0 00-.723-1.745 3.066 3.066 0 010-3.976 3.066 3.066 0 00.723-1.745 3.066 3.066 0 012.812-2.812zm7.44 5.252a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'bell'"
      d="M10 2a6 6 0 00-6 6v3.586l-.707.707A1 1 0 004 14h12a1 1 0 00.707-1.707L16 11.586V8a6 6 0 00-6-6zM10 18a3 3 0 01-3-3h6a3 3 0 01-3 3z"
    />

    <path
      v-if="name === 'book-open'"
      d="M9 4.804A7.968 7.968 0 005.5 4c-1.255 0-2.443.29-3.5.804v10A7.969 7.969 0 015.5 14c1.669 0 3.218.51 4.5 1.385A7.962 7.962 0 0114.5 14c1.255 0 2.443.29 3.5.804v-10A7.968 7.968 0 0014.5 4c-1.255 0-2.443.29-3.5.804V12a1 1 0 11-2 0V4.804z"
    />

    <path
      v-if="name === 'calendar'"
      fill-rule="evenodd"
      d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'cash'"
      fill-rule="evenodd"
      d="M4 4a2 2 0 00-2 2v4a2 2 0 002 2V6h10a2 2 0 00-2-2H4zm2 6a2 2 0 012-2h8a2 2 0 012 2v4a2 2 0 01-2 2H8a2 2 0 01-2-2v-4zm6 4a2 2 0 100-4 2 2 0 000 4z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'check'"
      fill-rule="evenodd"
      d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'chevron-down'"
      fill-rule="evenodd"
      d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'chevron-left'"
      fill-rule="evenodd"
      d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'chevron-right'"
      fill-rule="evenodd"
      d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z"
      clip-rule="evenodd"
    />

    <g v-if="name === 'clipboard-check'">
      <path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z" />
      <path
        fill-rule="evenodd"
        d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm9.707 5.707a1 1 0 00-1.414-1.414L9 12.586l-1.293-1.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
        clip-rule="evenodd"
      />
    </g>

    <g v-if="name === 'clipboard-copy'">
      <path d="M8 2a1 1 0 000 2h2a1 1 0 100-2H8z" />
      <path
        d="M3 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v6h-4.586l1.293-1.293a1 1 0 00-1.414-1.414l-3 3a1 1 0 000 1.414l3 3a1 1 0 001.414-1.414L10.414 13H15v3a2 2 0 01-2 2H5a2 2 0 01-2-2V5zM15 11h2a1 1 0 110 2h-2v-2z"
      />
    </g>

    <g v-if="name === 'clipboard-list'">
      <path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z" />
      <path
        fill-rule="evenodd"
        d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3zm-3 4a1 1 0 100 2h.01a1 1 0 100-2H7zm3 0a1 1 0 100 2h3a1 1 0 100-2h-3z"
        clip-rule="evenodd"
      />
    </g>

    <g v-if="name === 'clipboard'">
      <path d="M8 3a1 1 0 011-1h2a1 1 0 110 2H9a1 1 0 01-1-1z" />
      <path
        d="M6 3a2 2 0 00-2 2v11a2 2 0 002 2h8a2 2 0 002-2V5a2 2 0 00-2-2 3 3 0 01-3 3H9a3 3 0 01-3-3z"
      />
    </g>

    <path
      v-if="name === 'clock'"
      fill-rule="evenodd"
      d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'code'"
      fill-rule="evenodd"
      d="M12.316 3.051a1 1 0 01.633 1.265l-4 12a1 1 0 11-1.898-.632l4-12a1 1 0 011.265-.633zM5.707 6.293a1 1 0 010 1.414L3.414 10l2.293 2.293a1 1 0 11-1.414 1.414l-3-3a1 1 0 010-1.414l3-3a1 1 0 011.414 0zm8.586 0a1 1 0 011.414 0l3 3a1 1 0 010 1.414l-3 3a1 1 0 11-1.414-1.414L16.586 10l-2.293-2.293a1 1 0 010-1.414z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'codepen'"
      fill="currentColor"
      d="M8.21 12L6.88 12.89V11.11L8.21 12M11.47 9.82V7.34L7.31 10.12L9.16 11.36L11.47 9.82M16.7 10.12L12.53 7.34V9.82L14.84 11.36L16.7 10.12M7.31 13.88L11.47 16.66V14.18L9.16 12.64L7.31 13.88M12.53 14.18V16.66L16.7 13.88L14.84 12.64L12.53 14.18M12 10.74L10.12 12L12 13.26L13.88 12L12 10.74M22 12C22 17.5 17.5 22 12 22C6.5 22 2 17.5 2 12C2 6.5 6.5 2 12 2C17.5 2 22 6.5 22 12M18.18 10.12C18.18 10.09 18.18 10.07 18.18 10.05L18.17 10L18.17 10L18.16 9.95C18.15 9.94 18.15 9.93 18.14 9.91L18.13 9.89L18.11 9.85L18.1 9.83L18.08 9.8L18.06 9.77L18.03 9.74L18 9.72L18 9.7L17.96 9.68L17.95 9.67L12.3 5.91C12.12 5.79 11.89 5.79 11.71 5.91L6.05 9.67L6.05 9.68L6 9.7C6 9.71 6 9.72 6 9.72L5.97 9.74L5.94 9.77L5.93 9.8L5.9 9.83L5.89 9.85L5.87 9.89L5.86 9.91L5.84 9.95L5.84 10L5.83 10L5.82 10.05C5.82 10.07 5.82 10.09 5.82 10.12V13.88C5.82 13.91 5.82 13.93 5.82 13.95L5.83 14L5.84 14L5.84 14.05C5.85 14.06 5.85 14.07 5.86 14.09L5.87 14.11L5.89 14.15L5.9 14.17L5.92 14.2L5.94 14.23C5.95 14.24 5.96 14.25 5.97 14.26L6 14.28L6 14.3L6.04 14.32L6.05 14.33L11.71 18.1C11.79 18.16 11.9 18.18 12 18.18C12.1 18.18 12.21 18.15 12.3 18.1L17.95 14.33L17.96 14.32L18 14.3L18 14.28L18.03 14.26L18.06 14.23L18.08 14.2L18.1 14.17L18.11 14.15L18.13 14.11L18.14 14.09L18.16 14.05L18.16 14L18.17 14L18.18 13.95C18.18 13.93 18.18 13.91 18.18 13.88V10.12M17.12 12.89V11.11L15.79 12L17.12 12.89Z"
    />

    <path
      v-if="name === 'coffee'"
      d="M4 11H2a2 2 0 0 1-2-2V5c0-1.1.9-2 2-2h2V1h14v10a4 4 0 0 1-4 4H8a4 4 0 0 1-4-4zm0-2V5H2v4h2zm-2 8v-1h18v1l-4 2H6l-4-2z"
    />

    <path
      v-if="name === 'cog'"
      fill-rule="evenodd"
      d="M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.533 1.533 0 01.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'collection'"
      d="M7 3a1 1 0 000 2h6a1 1 0 100-2H7zM4 7a1 1 0 011-1h10a1 1 0 110 2H5a1 1 0 01-1-1zM2 11a2 2 0 012-2h12a2 2 0 012 2v4a2 2 0 01-2 2H4a2 2 0 01-2-2v-4z"
    />

    <g v-if="name === 'copy'">
      <path d="M8 3a1 1 0 011-1h2a1 1 0 110 2H9a1 1 0 01-1-1z" />
      <path
        d="M6 3a2 2 0 00-2 2v11a2 2 0 002 2h8a2 2 0 002-2V5a2 2 0 00-2-2 3 3 0 01-3 3H9a3 3 0 01-3-3z"
      />
    </g>

    <g v-if="name === 'credit-card'">
      <path d="M4 4a2 2 0 00-2 2v1h16V6a2 2 0 00-2-2H4z" />
      <path
        fill-rule="evenodd"
        d="M18 9H2v5a2 2 0 002 2h12a2 2 0 002-2V9zM4 13a1 1 0 011-1h1a1 1 0 110 2H5a1 1 0 01-1-1zm5-1a1 1 0 100 2h1a1 1 0 100-2H9z"
        clip-rule="evenodd"
      />
    </g>

    <path
      v-if="name === 'cursor-click'"
      fill-rule="evenodd"
      d="M6.672 1.911a1 1 0 10-1.932.518l.259.966a1 1 0 001.932-.518l-.26-.966zM2.429 4.74a1 1 0 10-.517 1.932l.966.259a1 1 0 00.517-1.932l-.966-.26zm8.814-.569a1 1 0 00-1.415-1.414l-.707.707a1 1 0 101.415 1.415l.707-.708zm-7.071 7.072l.707-.707A1 1 0 003.465 9.12l-.708.707a1 1 0 001.415 1.415zm3.2-5.171a1 1 0 00-1.3 1.3l4 10a1 1 0 001.823.075l1.38-2.759 3.018 3.02a1 1 0 001.414-1.415l-3.019-3.02 2.76-1.379a1 1 0 00-.076-1.822l-10-4z"
      clip-rule="evenodd"
    />

    <g v-if="name === 'database'">
      <path
        d="M3 12v3c0 1.657 3.134 3 7 3s7-1.343 7-3v-3c0 1.657-3.134 3-7 3s-7-1.343-7-3z"
      />
      <path
        d="M3 7v3c0 1.657 3.134 3 7 3s7-1.343 7-3V7c0 1.657-3.134 3-7 3S3 8.657 3 7z"
      />
      <path
        d="M17 5c0 1.657-3.134 3-7 3S3 6.657 3 5s3.134-3 7-3 7 1.343 7 3z"
      />
    </g>

    <path
      v-if="name === 'desktop-computer'"
      fill-rule="evenodd"
      d="M3 5a2 2 0 012-2h10a2 2 0 012 2v8a2 2 0 01-2 2h-2.22l.123.489.804.804A1 1 0 0113 18H7a1 1 0 01-.707-1.707l.804-.804L7.22 15H5a2 2 0 01-2-2V5zm5.771 7H5V5h10v7H8.771z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'device-mobile'"
      fill-rule="evenodd"
      d="M7 2a2 2 0 00-2 2v12a2 2 0 002 2h6a2 2 0 002-2V4a2 2 0 00-2-2H7zm3 14a1 1 0 100-2 1 1 0 000 2z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'device-tablet'"
      fill-rule="evenodd"
      d="M6 2a2 2 0 00-2 2v12a2 2 0 002 2h8a2 2 0 002-2V4a2 2 0 00-2-2H6zm4 14a1 1 0 100-2 1 1 0 000 2z"
      clip-rule="evenodd"
    />

    <g v-if="name === 'document-search'">
      <path
        d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2h-1.528A6 6 0 004 9.528V4z"
      />
      <path
        fill-rule="evenodd"
        d="M8 10a4 4 0 00-3.446 6.032l-1.261 1.26a1 1 0 101.414 1.415l1.261-1.261A4 4 0 108 10zm-2 4a2 2 0 114 0 2 2 0 01-4 0z"
        clip-rule="evenodd"
      />
    </g>

    <path
      v-if="name === 'dots-vertical'"
      d="M10 6a2 2 0 110-4 2 2 0 010 4zM10 12a2 2 0 110-4 2 2 0 010 4zM10 18a2 2 0 110-4 2 2 0 010 4z"
    />

    <g v-if="name === 'duplicate'">
      <path
        d="M7 9a2 2 0 012-2h6a2 2 0 012 2v6a2 2 0 01-2 2H9a2 2 0 01-2-2V9z"
      />
      <path d="M5 3a2 2 0 00-2 2v6a2 2 0 002 2V5h8a2 2 0 00-2-2H5z" />
    </g>

    <g v-if="name === 'email'">
      <path
        d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z"
      />
      <path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z" />
    </g>

    <path
      v-if="name === 'exclamation'"
      fill-rule="evenodd"
      d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'exclamation-circle'"
      fill-rule="evenodd"
      d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z"
      clip-rule="evenodd"
    />

    <g v-if="name === 'external-link'">
      <path
        d="M11 3a1 1 0 100 2h2.586l-6.293 6.293a1 1 0 101.414 1.414L15 6.414V9a1 1 0 102 0V4a1 1 0 00-1-1h-5z"
      />
      <path
        d="M5 5a2 2 0 00-2 2v8a2 2 0 002 2h8a2 2 0 002-2v-3a1 1 0 10-2 0v3H5V7h3a1 1 0 000-2H5z"
      />
    </g>

    <path
      v-if="name === 'family-restroom'"
      d="M15.984 3.984q0-0.797 0.586-1.383t1.43-0.586 1.43 0.586 0.586 1.383q0 0.844-0.586 1.43t-1.43 0.586-1.43-0.586-0.586-1.43zM20.016 21.984v-6h2.484l-2.531-7.594q-0.234-0.656-0.75-1.031t-1.172-0.375h-0.094q-0.656 0-1.172 0.375t-0.75 1.031l-0.844 2.578q0.797 0.422 1.313 1.242t0.516 1.805v7.969h3zM12.516 11.484q0.609 0 1.055-0.422t0.445-1.078q0-0.609-0.445-1.055t-1.055-0.445q-0.656 0-1.078 0.445t-0.422 1.055q0 0.656 0.422 1.078t1.078 0.422zM5.484 6q0.844 0 1.43-0.586t0.586-1.43q0-0.797-0.586-1.383t-1.43-0.586q-0.797 0-1.383 0.586t-0.586 1.383q0 0.844 0.586 1.43t1.383 0.586zM7.5 21.984v-6.984h1.5v-6q0-0.844-0.586-1.43t-1.43-0.586h-3q-0.797 0-1.383 0.586t-0.586 1.43v6h1.5v6.984h3.984zM14.016 21.984v-3.984h0.984v-3.984q0-0.609-0.445-1.055t-1.055-0.445h-2.016q-0.609 0-1.055 0.445t-0.445 1.055v3.984h1.031v3.984h3z"
    />

    <path
      v-if="name === 'film'"
      fill-rule="evenodd"
      d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H4zm3 2h6v4H7V5zm8 8v2h1v-2h-1zm-2-2H7v4h6v-4zm2 0h1V9h-1v2zm1-4V5h-1v2h1zM5 5v2H4V5h1zm0 4H4v2h1V9zm-1 4h1v2H4v-2z"
      clip-rule="evenodd"
    />

    <g v-if="name === 'folder-add'">
      <path
        d="M2 6a2 2 0 012-2h5l2 2h5a2 2 0 012 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2V6z"
      />
      <path
        stroke="#fff"
        stroke-linecap="round"
        stroke-linejoin="round"
        stroke-width="2"
        d="M8 11h4m-2-2v4"
      />
    </g>

    <g v-if="name === 'folder-download'">
      <path
        d="M2 6a2 2 0 012-2h5l2 2h5a2 2 0 012 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2V6z"
      />
      <path
        stroke="#fff"
        stroke-linecap="round"
        stroke-linejoin="round"
        stroke-width="2"
        d="M10 9v4m0 0l-2-2m2 2l2-2"
      />
    </g>

    <g
      v-if="name === 'folder-open'"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 20 20"
      fill="currentColor"
    >
      <path
        fill-rule="evenodd"
        d="M2 6a2 2 0 012-2h4l2 2h4a2 2 0 012 2v1H8a3 3 0 00-3 3v1.5a1.5 1.5 0 01-3 0V6z"
        clip-rule="evenodd"
      />
      <path
        d="M6 12a2 2 0 012-2h8a2 2 0 012 2v2a2 2 0 01-2 2H2h2a2 2 0 002-2v-2z"
      />
    </g>

    <g v-if="name === 'folder-remove'">
      <path
        d="M2 6a2 2 0 012-2h5l2 2h5a2 2 0 012 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2V6z"
      />
      <path
        stroke="#fff"
        stroke-linecap="round"
        stroke-linejoin="round"
        stroke-width="2"
        d="M8 11h4"
      />
    </g>

    <path
      v-if="name === 'folder'"
      d="M2 6a2 2 0 012-2h5l2 2h5a2 2 0 012 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2V6z"
    />

    <g v-if="name === 'gift'">
      <path
        fill-rule="evenodd"
        d="M5 5a3 3 0 015-2.236A3 3 0 0114.83 6H16a2 2 0 110 4h-5V9a1 1 0 10-2 0v1H4a2 2 0 110-4h1.17C5.06 5.687 5 5.35 5 5zm4 1V5a1 1 0 10-1 1h1zm3 0a1 1 0 10-1-1v1h1z"
        clip-rule="evenodd"
      />
      <path d="M9 11H3v5a2 2 0 002 2h4v-7zM11 18h4a2 2 0 002-2v-5h-6v7z" />
    </g>

    <path
      v-if="name === 'github'"
      d="M10 1.66667C8.90565 1.66667 7.82202 1.88222 6.81097 2.30101C5.79993 2.7198 4.88127 3.33363 4.10744 4.10745C2.54464 5.67025 1.66667 7.78987 1.66667 10C1.66667 13.6833 4.05833 16.8083 7.36667 17.9167C7.78334 17.9833 7.91667 17.725 7.91667 17.5C7.91667 17.3083 7.91667 16.7833 7.91667 16.0917C5.60833 16.5917 5.11667 14.975 5.11667 14.975C4.73333 14.0083 4.19167 13.75 4.19167 13.75C3.43333 13.2333 4.25 13.25 4.25 13.25C5.08333 13.3083 5.525 14.1083 5.525 14.1083C6.25 15.375 7.475 15 7.95 14.8C8.025 14.2583 8.24167 13.8917 8.475 13.6833C6.625 13.475 4.68333 12.7583 4.68333 9.58334C4.68333 8.65834 5 7.91667 5.54167 7.32501C5.45833 7.11667 5.16667 6.25001 5.625 5.12501C5.625 5.12501 6.325 4.90001 7.91667 5.97501C8.575 5.79167 9.29167 5.70001 10 5.70001C10.7083 5.70001 11.425 5.79167 12.0833 5.97501C13.675 4.90001 14.375 5.12501 14.375 5.12501C14.8333 6.25001 14.5417 7.11667 14.4583 7.32501C15 7.91667 15.3167 8.65834 15.3167 9.58334C15.3167 12.7667 13.3667 13.4667 11.5083 13.675C11.8083 13.9333 12.0833 14.4417 12.0833 15.2167C12.0833 16.3333 12.0833 17.2333 12.0833 17.5C12.0833 17.725 12.2167 17.9917 12.6417 17.9167C15.95 16.8 18.3333 13.6833 18.3333 10C18.3333 8.90566 18.1178 7.82202 17.699 6.81098C17.2802 5.79993 16.6664 4.88127 15.8926 4.10745C15.1187 3.33363 14.2001 2.7198 13.189 2.30101C12.178 1.88222 11.0944 1.66667 10 1.66667V1.66667Z"
    />

    <path
      v-if="name === 'github-2'"
      d="M10 0a10 10 0 0 0-3.16 19.49c.5.1.68-.22.68-.48l-.01-1.7c-2.78.6-3.37-1.34-3.37-1.34-.46-1.16-1.11-1.47-1.11-1.47-.9-.62.07-.6.07-.6 1 .07 1.53 1.03 1.53 1.03.9 1.52 2.34 1.08 2.91.83.1-.65.35-1.09.63-1.34-2.22-.25-4.55-1.11-4.55-4.94 0-1.1.39-1.99 1.03-2.69a3.6 3.6 0 0 1 .1-2.64s.84-.27 2.75 1.02a9.58 9.58 0 0 1 5 0c1.91-1.3 2.75-1.02 2.75-1.02.55 1.37.2 2.4.1 2.64.64.7 1.03 1.6 1.03 2.69 0 3.84-2.34 4.68-4.57 4.93.36.31.68.92.68 1.85l-.01 2.75c0 .26.18.58.69.48A10 10 0 0 0 10 0"
    />

    <path
      v-if="name === 'handshake'"
      fill="currentColor"
      d="M21.71 8.71C22.96 7.46 22.39 6 21.71 5.29L18.71 2.29C17.45 1.04 16 1.61 15.29 2.29L13.59 4H11C9.1 4 8 5 7.44 6.15L3 10.59V14.59L2.29 15.29C1.04 16.55 1.61 18 2.29 18.71L5.29 21.71C5.83 22.25 6.41 22.45 6.96 22.45C7.67 22.45 8.32 22.1 8.71 21.71L11.41 19H15C16.7 19 17.56 17.94 17.87 16.9C19 16.6 19.62 15.74 19.87 14.9C21.42 14.5 22 13.03 22 12V9H21.41L21.71 8.71M20 12C20 12.45 19.81 13 19 13L18 13L18 14C18 14.45 17.81 15 17 15L16 15L16 16C16 16.45 15.81 17 15 17H10.59L7.31 20.28C7 20.57 6.82 20.4 6.71 20.29L3.72 17.31C3.43 17 3.6 16.82 3.71 16.71L5 15.41V11.41L7 9.41V11C7 12.21 7.8 14 10 14S13 12.21 13 11H20V12M20.29 7.29L18.59 9H11V11C11 11.45 10.81 12 10 12S9 11.45 9 11V8C9 7.54 9.17 6 11 6H14.41L16.69 3.72C17 3.43 17.18 3.6 17.29 3.71L20.28 6.69C20.57 7 20.4 7.18 20.29 7.29Z"
    />

    <path
      v-if="name === 'heart'"
      fill-rule="evenodd"
      d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'history'"
      fill="currentColor"
      d="M13.5,8H12V13L16.28,15.54L17,14.33L13.5,12.25V8M13,3A9,9 0 0,0 4,12H1L4.96,16.03L9,12H6A7,7 0 0,1 13,5A7,7 0 0,1 20,12A7,7 0 0,1 13,19C11.07,19 9.32,18.21 8.06,16.94L6.64,18.36C8.27,20 10.5,21 13,21A9,9 0 0,0 22,12A9,9 0 0,0 13,3"
    />

    <path
      v-if="name === 'home-heart'"
      fill="currentColor"
      d="M2,12L12,3L22,12H19V20H5V12H2M12,18L12.72,17.34C15.3,15 17,13.46 17,11.57C17,10.03 15.79,8.82 14.25,8.82C13.38,8.82 12.55,9.23 12,9.87C11.45,9.23 10.62,8.82 9.75,8.82C8.21,8.82 7,10.03 7,11.57C7,13.46 8.7,15 11.28,17.34L12,18Z"
    />

    <path
      v-if="name === 'home'"
      d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z"
    />

    <path
      v-if="name === 'identification'"
      fill-rule="evenodd"
      d="M10 2a1 1 0 00-1 1v1a1 1 0 002 0V3a1 1 0 00-1-1zM4 4h3a3 3 0 006 0h3a2 2 0 012 2v9a2 2 0 01-2 2H4a2 2 0 01-2-2V6a2 2 0 012-2zm2.5 7a1.5 1.5 0 100-3 1.5 1.5 0 000 3zm2.45 4a2.5 2.5 0 10-4.9 0h4.9zM12 9a1 1 0 100 2h3a1 1 0 100-2h-3zm-1 4a1 1 0 011-1h2a1 1 0 110 2h-2a1 1 0 01-1-1z"
      clip-rule="evenodd"
    />

    <g v-if="name === 'inbox-in'">
      <path
        d="M8.707 7.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l2-2a1 1 0 00-1.414-1.414L11 7.586V3a1 1 0 10-2 0v4.586l-.293-.293z"
      />
      <path
        d="M3 5a2 2 0 012-2h1a1 1 0 010 2H5v7h2l1 2h4l1-2h2V5h-1a1 1 0 110-2h1a2 2 0 012 2v10a2 2 0 01-2 2H5a2 2 0 01-2-2V5z"
      />
    </g>

    <path
      v-if="name === 'inbox'"
      fill-rule="evenodd"
      d="M5 3a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V5a2 2 0 00-2-2H5zm0 2h10v7h-2l-1 2H8l-1-2H5V5z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'filter'"
      fill-rule="evenodd"
      d="M3 3a1 1 0 011-1h12a1 1 0 011 1v3a1 1 0 01-.293.707L12 11.414V15a1 1 0 01-.293.707l-2 2A1 1 0 018 17v-5.586L3.293 6.707A1 1 0 013 6V3z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'key'"
      fill-rule="evenodd"
      d="M18 8a6 6 0 01-7.743 5.743L10 14l-1 1-1 1H6v2H2v-4l4.257-4.257A6 6 0 1118 8zm-6-4a1 1 0 100 2 2 2 0 012 2 1 1 0 102 0 4 4 0 00-4-4z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'library'"
      fill-rule="evenodd"
      d="M10.496 2.132a1 1 0 00-.992 0l-7 4A1 1 0 003 8v7a1 1 0 100 2h14a1 1 0 100-2V8a1 1 0 00.496-1.868l-7-4zM6 9a1 1 0 00-1 1v3a1 1 0 102 0v-3a1 1 0 00-1-1zm3 1a1 1 0 012 0v3a1 1 0 11-2 0v-3zm5-1a1 1 0 00-1 1v3a1 1 0 102 0v-3a1 1 0 00-1-1z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'linkedin'"
      d="M15.8333 2.5C16.2754 2.5 16.6993 2.67559 17.0118 2.98816C17.3244 3.30072 17.5 3.72464 17.5 4.16667V15.8333C17.5 16.2754 17.3244 16.6993 17.0118 17.0118C16.6993 17.3244 16.2754 17.5 15.8333 17.5H4.16667C3.72464 17.5 3.30072 17.3244 2.98816 17.0118C2.67559 16.6993 2.5 16.2754 2.5 15.8333V4.16667C2.5 3.72464 2.67559 3.30072 2.98816 2.98816C3.30072 2.67559 3.72464 2.5 4.16667 2.5H15.8333ZM15.4167 15.4167V11C15.4167 10.2795 15.1304 9.5885 14.621 9.07903C14.1115 8.56955 13.4205 8.28333 12.7 8.28333C11.9917 8.28333 11.1667 8.71667 10.7667 9.36667V8.44167H8.44167V15.4167H10.7667V11.3083C10.7667 10.6667 11.2833 10.1417 11.925 10.1417C12.2344 10.1417 12.5312 10.2646 12.75 10.4834C12.9688 10.7022 13.0917 10.9989 13.0917 11.3083V15.4167H15.4167ZM5.73333 7.13333C6.10464 7.13333 6.46073 6.98583 6.72328 6.72328C6.98583 6.46073 7.13333 6.10464 7.13333 5.73333C7.13333 4.95833 6.50833 4.325 5.73333 4.325C5.35982 4.325 5.0016 4.47338 4.73749 4.73749C4.47338 5.0016 4.325 5.35982 4.325 5.73333C4.325 6.50833 4.95833 7.13333 5.73333 7.13333ZM6.89167 15.4167V8.44167H4.58333V15.4167H6.89167Z"
    />

    <path
      v-if="name === 'mail-open'"
      fill-rule="evenodd"
      d="M2.94 6.412A2 2 0 002 8.108V16a2 2 0 002 2h12a2 2 0 002-2V8.108a2 2 0 00-.94-1.696l-6-3.75a2 2 0 00-2.12 0l-6 3.75zm2.615 2.423a1 1 0 10-1.11 1.664l5 3.333a1 1 0 001.11 0l5-3.333a1 1 0 00-1.11-1.664L10 11.798 5.555 8.835z"
      clip-rule="evenodd"
    />

    <g v-if="name === 'mail'">
      <path
        d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z"
      />
      <path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z" />
    </g>

    <path
      v-if="name === 'map'"
      fill-rule="evenodd"
      d="M12 1.586l-4 4v12.828l4-4V1.586zM3.707 3.293A1 1 0 002 4v10a1 1 0 00.293.707L6 18.414V5.586L3.707 3.293zM17.707 5.293L14 1.586v12.828l2.293 2.293A1 1 0 0018 16V6a1 1 0 00-.293-.707z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'menu'"
      fill-rule="evenodd"
      d="M3 5a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zM3 15a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'minus-circle'"
      fill-rule="evenodd"
      d="M10 18a8 8 0 100-16 8 8 0 000 16zM7 9a1 1 0 000 2h6a1 1 0 100-2H7z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'minus-sm'"
      fill-rule="evenodd"
      d="M5 10a1 1 0 011-1h8a1 1 0 110 2H6a1 1 0 01-1-1z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'minus'"
      fill-rule="evenodd"
      d="M3 10a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'moon'"
      d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z"
    />

    <path
      v-if="name === 'musical-note'"
      d="M18 3a1 1 0 00-1.196-.98l-10 2A1 1 0 006 5v9.114A4.369 4.369 0 005 14c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V7.82l8-1.6v5.894A4.37 4.37 0 0015 12c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V3z"
    />

    <g v-if="name === 'newspaper'">
      <path
        fill-rule="evenodd"
        d="M2 5a2 2 0 012-2h8a2 2 0 012 2v10a2 2 0 002 2H4a2 2 0 01-2-2V5zm3 1h6v4H5V6zm6 6H5v2h6v-2z"
        clip-rule="evenodd"
      />
      <path d="M15 7h1a2 2 0 012 2v5.5a1.5 1.5 0 01-3 0V7z" />
    </g>

    <g v-if="name === 'pencil-alt'">
      <path
        d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z"
      />
      <path
        fill-rule="evenodd"
        d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z"
        clip-rule="evenodd"
      />
    </g>

    <path
      v-if="name === 'pencil'"
      d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"
    />

    <path
      v-if="name === 'play'"
      fill-rule="evenodd"
      d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'play-circle'"
      fill-rule="evenodd"
      d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11a1 1 0 10-2 0v2H7a1 1 0 100 2h2v2a1 1 0 102 0v-2h2a1 1 0 100-2h-2V7z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'plus-sm'"
      fill-rule="evenodd"
      d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'plus'"
      fill-rule="evenodd"
      d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'puzzle'"
      d="M10 3.5a1.5 1.5 0 013 0V4a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-.5a1.5 1.5 0 000 3h.5a1 1 0 011 1v3a1 1 0 01-1 1h-3a1 1 0 01-1-1v-.5a1.5 1.5 0 00-3 0v.5a1 1 0 01-1 1H6a1 1 0 01-1-1v-3a1 1 0 00-1-1h-.5a1.5 1.5 0 010-3H4a1 1 0 001-1V6a1 1 0 011-1h3a1 1 0 001-1v-.5z"
    />

    <path
      v-if="name === 'office-building'"
      fill-rule="evenodd"
      d="M4 4a2 2 0 012-2h8a2 2 0 012 2v12a1 1 0 110 2h-3a1 1 0 01-1-1v-2a1 1 0 00-1-1H9a1 1 0 00-1 1v2a1 1 0 01-1 1H4a1 1 0 110-2V4zm3 1h2v2H7V5zm2 4H7v2h2V9zm2-4h2v2h-2V5zm2 4h-2v2h2V9z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'question-mark-circle'"
      fill-rule="evenodd"
      d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'refresh'"
      fill-rule="evenodd"
      d="M4 2a1 1 0 011 1v2.101a7.002 7.002 0 0111.601 2.566 1 1 0 11-1.885.666A5.002 5.002 0 005.999 7H9a1 1 0 010 2H4a1 1 0 01-1-1V3a1 1 0 011-1zm.008 9.057a1 1 0 011.276.61A5.002 5.002 0 0014.001 13H11a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0v-2.101a7.002 7.002 0 01-11.601-2.566 1 1 0 01.61-1.276z"
      clip-rule="evenodd"
    />

    <g v-if="name === 'rss'">
      <path
        d="M5 3a1 1 0 000 2c5.523 0 10 4.477 10 10a1 1 0 102 0C17 8.373 11.627 3 5 3z"
      />
      <path
        d="M4 9a1 1 0 011-1 7 7 0 017 7 1 1 0 11-2 0 5 5 0 00-5-5 1 1 0 01-1-1zM3 15a2 2 0 114 0 2 2 0 01-4 0z"
      />
    </g>

    <path
      v-if="name === 'save'"
      d="M7.707 10.293a1 1 0 10-1.414 1.414l3 3a1 1 0 001.414 0l3-3a1 1 0 00-1.414-1.414L11 11.586V6h5a2 2 0 012 2v7a2 2 0 01-2 2H4a2 2 0 01-2-2V8a2 2 0 012-2h5v5.586l-1.293-1.293zM9 4a1 1 0 012 0v2H9V4z"
    />

    <path
      v-if="name === 'search'"
      fill-rule="evenodd"
      d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'sparkles'"
      fill-rule="evenodd"
      d="M5 2a1 1 0 011 1v1h1a1 1 0 010 2H6v1a1 1 0 01-2 0V6H3a1 1 0 010-2h1V3a1 1 0 011-1zm0 10a1 1 0 011 1v1h1a1 1 0 110 2H6v1a1 1 0 11-2 0v-1H3a1 1 0 110-2h1v-1a1 1 0 011-1zM12 2a1 1 0 01.967.744L14.146 7.2 17.5 9.134a1 1 0 010 1.732l-3.354 1.935-1.18 4.455a1 1 0 01-1.933 0L9.854 12.8 6.5 10.866a1 1 0 010-1.732l3.354-1.935 1.18-4.455A1 1 0 0112 2z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'speaker-phone'"
      fill-rule="evenodd"
      d="M18 3a1 1 0 00-1.447-.894L8.763 6H5a3 3 0 000 6h.28l1.771 5.316A1 1 0 008 18h1a1 1 0 001-1v-4.382l6.553 3.276A1 1 0 0018 15V3z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'star'"
      d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"
    />

    <path
      v-if="name === 'user'"
      fill-rule="evenodd"
      d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'volunteer'"
      fill="currentColor"
      d="M20 17Q20.86 17 21.45 17.6T22.03 19L14 22L7 20V11H8.95L16.22 13.69Q17 14 17 14.81 17 15.28 16.66 15.63T15.8 16H13L11.25 15.33L10.92 16.27L13 17H20M16 3.23Q17.06 2 18.7 2 20.06 2 21 3T22 5.3Q22 6.33 21 7.76T19.03 10.15 16 13Q13.92 11.11 12.94 10.15T10.97 7.76 10 5.3Q10 3.94 10.97 3T13.31 2Q14.91 2 16 3.23M.984 11H5V22H.984V11Z"
    />

    <path
      v-if="name === 'tag'"
      fill-rule="evenodd"
      d="M17.707 9.293a1 1 0 010 1.414l-7 7a1 1 0 01-1.414 0l-7-7A.997.997 0 012 10V5a3 3 0 013-3h5c.256 0 .512.098.707.293l7 7zM5 6a1 1 0 100-2 1 1 0 000 2z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'table'"
      fill-rule="evenodd"
      d="M5 4a3 3 0 00-3 3v6a3 3 0 003 3h10a3 3 0 003-3V7a3 3 0 00-3-3H5zm-1 9v-1h5v2H5a1 1 0 01-1-1zm7 1h4a1 1 0 001-1v-1h-5v2zm0-4h5V8h-5v2zM9 8H4v2h5V8z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'template'"
      d="M3 4a1 1 0 011-1h12a1 1 0 011 1v2a1 1 0 01-1 1H4a1 1 0 01-1-1V4zM3 10a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H4a1 1 0 01-1-1v-6zM14 9a1 1 0 00-1 1v6a1 1 0 001 1h2a1 1 0 001-1v-6a1 1 0 00-1-1h-2z"
    />

    <path
      v-if="name === 'terminal'"
      fill-rule="evenodd"
      d="M2 5a2 2 0 012-2h12a2 2 0 012 2v10a2 2 0 01-2 2H4a2 2 0 01-2-2V5zm3.293 1.293a1 1 0 011.414 0l3 3a1 1 0 010 1.414l-3 3a1 1 0 01-1.414-1.414L7.586 10 5.293 7.707a1 1 0 010-1.414zM11 12a1 1 0 100 2h3a1 1 0 100-2h-3z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'user-add'"
      d="M8 9a3 3 0 100-6 3 3 0 000 6zM8 11a6 6 0 016 6H2a6 6 0 016-6zM16 7a1 1 0 10-2 0v1h-1a1 1 0 100 2h1v1a1 1 0 102 0v-1h1a1 1 0 100-2h-1V7z"
    />

    <path
      v-if="name === 'user-circle'"
      fill-rule="evenodd"
      d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-6-3a2 2 0 11-4 0 2 2 0 014 0zm-2 4a5 5 0 00-4.546 2.916A5.986 5.986 0 0010 16a5.986 5.986 0 004.546-2.084A5 5 0 0010 11z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'twitter'"
      d="M6.29 18.25c7.55 0 11.67-6.25 11.67-11.67v-.53c.8-.59 1.49-1.3 2.04-2.13-.75.33-1.54.55-2.36.65a4.12 4.12 0 0 0 1.8-2.27c-.8.48-1.68.81-2.6 1a4.1 4.1 0 0 0-7 3.74 11.65 11.65 0 0 1-8.45-4.3 4.1 4.1 0 0 0 1.27 5.49C2.01 8.2 1.37 8.03.8 7.7v.05a4.1 4.1 0 0 0 3.3 4.03 4.1 4.1 0 0 1-1.86.07 4.1 4.1 0 0 0 3.83 2.85A8.23 8.23 0 0 1 0 16.4a11.62 11.62 0 0 0 6.29 1.84"
    />

    <path
      v-if="name === 'tiktok'"
      d="M17 9.15126C15.7221 9.15435 14.4756 8.75201 13.4364 8.00107V13.2383C13.4361 14.2083 13.1424 15.1551 12.5947 15.9521C12.047 16.749 11.2713 17.3581 10.3714 17.698C9.47155 18.0379 8.49034 18.0923 7.55899 17.854C6.62765 17.6157 5.79057 17.096 5.15968 16.3644C4.52879 15.6328 4.13417 14.7242 4.02858 13.7601C3.92299 12.796 4.11147 11.8223 4.56881 10.9692C5.02615 10.1161 5.73055 9.42437 6.58783 8.98639C7.44511 8.54841 8.4144 8.3851 9.36609 8.51829V11.1525C8.93059 11.0142 8.46295 11.0183 8.02994 11.1644C7.59692 11.3104 7.22069 11.5908 6.95496 11.9656C6.68923 12.3404 6.5476 12.7904 6.55029 13.2513C6.55298 13.7122 6.69985 14.1604 6.96993 14.532C7.24002 14.9036 7.6195 15.1796 8.05418 15.3204C8.48887 15.4613 8.95653 15.4599 9.39038 15.3164C9.82424 15.173 10.2021 14.8948 10.47 14.5216C10.7379 14.1484 10.8821 13.6992 10.8821 13.2383V3H13.4364C13.4347 3.21777 13.4527 3.43525 13.4904 3.64968V3.64968C13.5792 4.12836 13.7637 4.58372 14.0328 4.98793C14.3019 5.39213 14.6498 5.73667 15.0552 6.00047C15.6321 6.38555 16.3084 6.5908 17 6.59066V9.15126Z"
    />

    <path
      v-if="name === 'unsplash'"
      d="M17 9.19281V17H3V9.19281H7.41031V13.0966H12.5897V9.19281H17ZM12.5897 3H7.41031V6.90344H12.5897V3Z"
    />

    <path
      v-if="name === 'user-remove'"
      d="M11 6a3 3 0 11-6 0 3 3 0 016 0zM14 17a6 6 0 00-12 0h12zM13 8a1 1 0 100 2h4a1 1 0 100-2h-4z"
    />

    <path
      v-if="name === 'users'"
      d="M9 6a3 3 0 11-6 0 3 3 0 016 0zM17 6a3 3 0 11-6 0 3 3 0 016 0zM12.93 17c.046-.327.07-.66.07-1a6.97 6.97 0 00-1.5-4.33A5 5 0 0119 16v1h-6.07zM6 11a5 5 0 015 5v1H1v-1a5 5 0 015-5z"
    />

    <g v-if="name === 'vti'">
      <path
        d="M15.6033 19.6859C15.6565 16.1349 15.6096 10.2382 15.599 9.01071C17.0252 8.29332 17.9787 7.1269 17.9787 5.78624C17.9787 5.26749 17.8214 4.80378 17.5637 4.36377H23.9062C23.8995 4.37825 23.892 4.39369 23.884 4.41008C23.8218 4.5375 23.7303 4.71808 23.6135 4.94462C23.3796 5.39746 23.0452 6.0313 22.6436 6.78525C21.8402 8.29301 20.7687 10.2795 19.6954 12.2553C18.622 14.2311 17.5471 16.196 16.7374 17.6604C16.3325 18.3927 15.9941 18.9995 15.7554 19.4199C15.6989 19.5193 15.6481 19.6084 15.6033 19.6859Z"
        fill="currentColor"
      />
      <path
        d="M6.06596 5.7867C6.06596 7.12746 7.01982 8.29398 8.44617 9.01132C8.43527 10.2387 8.38818 16.1353 8.44186 19.6861C8.39714 19.6086 8.34631 19.5198 8.28982 19.4204C8.05108 19 7.71264 18.3932 7.3077 17.6609C6.49793 16.1966 5.42297 14.2318 4.34958 12.256C3.27616 10.2802 2.20446 8.29364 1.40119 6.78586C0.999524 6.03191 0.665143 5.39802 0.431299 4.94518C0.314304 4.71862 0.222818 4.53804 0.160725 4.41062C0.152713 4.39421 0.145256 4.37874 0.138306 4.36426H6.48109C6.22357 4.80419 6.06596 5.26802 6.06596 5.7867Z"
        fill="currentColor"
      />
      <path
        d="M12.0448 10.0543C12.0498 10.0544 12.0548 10.0545 12.0598 10.0546C12.0878 10.0553 12.1175 10.056 12.1456 10.056C12.8421 10.056 13.5286 9.84051 14.1355 9.65008C14.2028 9.62894 14.2692 9.60809 14.3345 9.58789C14.3353 9.68896 14.3362 9.83255 14.3374 10.0046C14.3397 10.3505 14.3427 10.8111 14.3457 11.2717L14.3539 12.5379L14.3565 12.955L14.3572 13.0719L14.3575 13.1026L14.3575 13.1105V13.1119L14.1978 23.3308V23.3313C14.1971 23.401 14.157 23.4768 14.0474 23.5564C13.9377 23.6361 13.7735 23.7073 13.5653 23.7652C13.1502 23.8808 12.5915 23.9351 12.0312 23.9216L11.9077 23.9187V23.924C11.3854 23.9286 10.8722 23.8745 10.4839 23.7671C10.2744 23.7092 10.1092 23.638 9.99875 23.5581C9.88844 23.4782 9.84821 23.402 9.84758 23.3325V23.3315L9.68744 13.1124L9.68746 13.111L9.68751 13.1032L9.6877 13.0724L9.68845 12.9555L9.69111 12.5384L9.69922 11.2722C9.70221 10.8117 9.7052 10.351 9.70754 10.0051C9.70868 9.83303 9.70967 9.68942 9.71039 9.58835C9.77649 9.60879 9.8437 9.62986 9.91187 9.65126C10.5183 9.8416 11.2038 10.0567 11.8992 10.0567C11.9234 10.0567 11.9504 10.0561 11.9754 10.0555C11.9909 10.0551 12.0056 10.0548 12.0183 10.0546L12.0448 10.0543Z"
        fill="currentColor"
      />
      <path
        d="M12.0281 8.70426L11.934 8.70443C10.7844 8.68773 9.75101 8.36636 9.00218 7.86098C8.23223 7.34133 7.77393 6.63628 7.77393 5.87304C7.77393 5.1111 8.23071 4.40711 8.99841 3.88773C9.76541 3.36881 10.8321 3.04321 12.0169 3.04116L12.1076 3.04102C13.2586 3.05714 14.2933 3.37868 15.0428 3.88454C15.8127 4.40421 16.271 5.10929 16.271 5.87253C16.271 6.63447 15.8142 7.33846 15.0464 7.85784C14.2794 8.37674 13.2127 8.70231 12.0281 8.70426Z"
        fill="currentColor"
      />
      <path
        d="M0.239563 3.3495C0.284836 3.31091 0.337686 3.26866 0.398379 3.22302C0.579349 3.08691 0.821977 2.92675 1.12296 2.75169C1.72475 2.40169 2.55559 1.9947 3.58467 1.60373C5.64269 0.821785 8.48939 0.105386 11.8765 0.0348949C11.9058 0.0343157 11.9332 0.0342195 11.9617 0.034123C11.9797 0.0340506 11.9982 0.0340021 12.0179 0.0338091L12.0544 0.0334473C12.0653 0.0334955 12.0758 0.0335193 12.0861 0.0335435C12.1129 0.0335917 12.1387 0.0336403 12.1683 0.0342436C15.5555 0.104711 18.4023 0.821085 20.4603 1.603C21.4894 1.994 22.3202 2.40099 22.9221 2.75101C23.223 2.92607 23.4656 3.08624 23.6466 3.22237C23.7071 3.26803 23.76 3.31031 23.8054 3.3489C23.7955 3.34897 23.7856 3.34902 23.7752 3.34909C23.5761 3.35027 23.2911 3.3515 22.9491 3.35273C22.2647 3.35519 21.3516 3.3577 20.4366 3.35992C19.5218 3.36214 18.6054 3.36408 17.9148 3.3654C17.5695 3.36605 17.2806 3.36656 17.0766 3.36688L16.8379 3.36719L16.7946 3.36721C15.73 2.40568 14.0499 1.79189 12.1677 1.79189C12.134 1.79189 12.0994 1.79281 12.0668 1.79367C12.0527 1.79406 12.0389 1.79442 12.0258 1.79471L12.0083 1.79507C11.9992 1.79483 11.9898 1.79457 11.9803 1.7943C11.9468 1.79336 11.911 1.79235 11.877 1.79235C9.99514 1.79235 8.31511 2.40638 7.25031 3.36786L7.20706 3.36784L6.96832 3.3675C6.76435 3.36716 6.47551 3.36666 6.13019 3.36601C5.43959 3.36468 4.52319 3.36275 3.60834 3.36053C2.69348 3.35831 1.78018 3.35582 1.09581 3.35334C0.75361 3.35211 0.468726 3.35087 0.269536 3.34969C0.259303 3.34962 0.249313 3.34957 0.239563 3.3495Z"
        fill="currentColor"
      />
    </g>

    <path
      v-if="name === 'x'"
      fill-rule="evenodd"
      d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
      clip-rule="evenodd"
    />

    <path
      v-if="name === 'view-grid-add'"
      d="M5 3a2 2 0 00-2 2v2a2 2 0 002 2h2a2 2 0 002-2V5a2 2 0 00-2-2H5zM5 11a2 2 0 00-2 2v2a2 2 0 002 2h2a2 2 0 002-2v-2a2 2 0 00-2-2H5zM11 5a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V5zM14 11a1 1 0 011 1v1h1a1 1 0 110 2h-1v1a1 1 0 11-2 0v-1h-1a1 1 0 110-2h1v-1a1 1 0 011-1z"
    />

    <path
      v-if="name === 'volunteer-activism'"
      d="M20.016 17.016q0.844 0 1.43 0.586t0.586 1.383l-8.016 3-7.031-1.969v-9h1.969l7.266 2.672q0.797 0.328 0.797 1.125 0 0.469-0.352 0.82t-0.867 0.352h-2.813l-1.734-0.656-0.328 0.938 2.063 0.75h7.031zM15.984 3.234q1.078-1.219 2.719-1.219 1.359 0 2.32 0.961t0.961 2.32q0 1.031-0.984 2.461t-1.969 2.391-3.047 2.836q-2.063-1.875-3.047-2.836t-1.969-2.391-0.984-2.461q0-1.359 0.984-2.32t2.344-0.961q1.594 0 2.672 1.219zM0.984 11.016h4.031v10.969h-4.031v-10.969z"
    />

    <g v-if="name === 'vue'">
      <path
        d="M14.6158 2.08113L11.9708 6.66242L9.32582 2.08113H0.517578L11.9708 21.9189L23.4241 2.08113H14.6158Z"
        fill="#41B883"
      />
      <path
        d="M14.6158 2.08118L11.9708 6.66248L9.32582 2.08118H5.09888L11.9708 13.9836L18.8428 2.08118H14.6158Z"
        fill="#34495E"
      />
    </g>
  </svg>
</template>
